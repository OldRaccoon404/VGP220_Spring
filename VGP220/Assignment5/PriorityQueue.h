...
public:
	...
	void Enqueue(Data data)
	{
		...
		std::cout << "Heap is full! Not possible to Insert! \n";
...
private:
	...
	void Heapfy(int index)
	{
		...
		if (leftIndex < mSize-1 && mDataArray[leftIndex].priority > mDataArray[biggst].priority)
		{
			biggst = leftIndex;
		}

		if (rightIndex < mSize - 1 && mDataArray[rightIndex].priority > mDataArray[biggst].priority)
		{
			biggst = rightIndex;
		...
		if (biggst != index)
		{
			swap(&mDataArray[index], &mDataArray[biggst]);
